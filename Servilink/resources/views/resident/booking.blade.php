                ...
                <div class="col-lg-6 col-sm-6 col-xs-12">
                    <div class="white-box"
                        ...
                        <ul class="list-inline two-part">
                            <li><span class="t_space">{{ $bookcnt }}</span></li>
                        </ul>
                    </div>
                </div>
                            ...
                            <div>
                                <div class="form-group col-md-12">
                                    <select name="venue" id="venue"
                                        class="form-control @error('space') is-invalid @enderror" required>
                                        <option value="0">Select Venue/ Location</option>
                                        @foreach ($spaces as $space)
                                            <option value="{{ $space->id }}" data-amount="{{ $space->cost }}">
                                                {{ $space->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('space')
    ...
    <script>
        $(document).ready(function(e) {
            ...
            $('#book-space').submit(function(e) {
                ...
                var url = "{{ route('store.booking') }}";
                if ($('#data_id').val() !== "0") {
                    url = "{{ route('edit.booking') }}";
                }
                $.ajax({
                    method: 'POST',
                    url: url,
